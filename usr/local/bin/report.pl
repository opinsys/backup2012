...
my @hosts_according_to_total = (@hosts[@permutation])[0..$SLICES];
...
my @hosts_according_to_transferred = (@hosts[@permutation])[0..$SLICES];
...
push @images, "<li><a class='graph' href='/$filename'><img class='thumb' src='/$filename' alt='Total data'></a></li>";
...
push @images, "<li><a class='graph' href='/$filename'><img class='thumb' src='/$filename' alt='Transferred data'></a></li>";
open(IMG, ">$WWWDIR/$filename") or die$!;
binmode IMG;
print IMG $gd2->png;
close IMG;
